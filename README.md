...
- Cluster Akka: optionnel, nécessite deux nœuds cohérents (2551/2552) si activé; sinon, privilégier provider local/test.

# À faire / manquants
- Découverte de services (Eureka) au lieu des URLs statiques.
- Appli de démo distribuée demandée (ex: restaurant/clients) avec 2 microservices distincts + scripts/tests d’intégration.
...
curl -s http://localhost:8080/api/metrics/actors
curl -s http://localhost:8080/api/metrics/actors/detail
curl -s http://localhost:8080/api/metrics/events
curl -s http://localhost:8080/api/metrics/alerts
curl -N http://localhost:8080/api/metrics/stream   # SSE
